...
@Slf4j
public class JwtTokenForPunchInterceptor extends HandlerInterceptorAdapter {
  ...
  public boolean preHandle(HttpServletRequest request, HttpServletResponse response,
    ...
    String key = RedisConstants.USER_LOGIN_KEY_PREFIX + claims.getId();
    // 是否有效 > 超时
    ...
    UserDtlVO userDtlVo = (UserDtlVO) redisUtils.get(key);
    // 登录时候， 存储一份用户的token， 一个用户只允许，

    // 是否存在
    if (null == userDtlVo) {
      return ResponseUtils.output(response, ErrorCodes.token_err);
    }
    if (UserStatusCodes.normal.getCode() != userDtlVo.getStatus()) {
      return ResponseUtils.output(response, ErrorCodes.account_lock_error);
    }
    request.setAttribute("currUserDtlVo", userDtlVo);
    log.debug("当前请求的用户 {}", userDtlVo);
    return true;
  }
}
