...
package cn.birdplanet.toolkit.core.idwork;
 ...
 * <br> 1位标识，由于long基本类型在Java中是带符号的，最高位是符号位，正数是0，负数是1，所以id一般是正数，最高位是0<br>
...
public final class SnowflakeUtils {
   ...
   * 开始时间截 (2019-05-08)
   */
  private final long twepoch = 1557330655019L;
   ...
   * 时间回调 单位： ms
   */
  private long clockBackOffsetMax = 15;
  ...
  private void throwClockBackwardsEx(long timestamp) {
    LOGGER.error("时钟向后调整. 拒绝生成{}毫秒内的ID", lastTimestamp - timestamp);
  }
  ...
  public Map<String, Object> parse(long uid) {
    ...
    String id = Long.toBinaryString(uid);
    int len = id.length();
    ...
    String sequence = id.substring(sequenceStart.intValue(), len);
    String workerId =
        sequenceStart == 0 ? "0" : id.substring(workerStart.intValue(), sequenceStart.intValue());
    String dataCenterId =
        workerStart == 0 ? "0" : id.substring(timeStart.intValue(), workerStart.intValue());
    String time = timeStart == 0 ? "0" : id.substring(0, timeStart.intValue());
    ...
    long timeLong = diffTime + timeStart + twepoch;

    Map<String, Object> retMap = Maps.newHashMapWithExpectedSize(6);
    retMap.put("uid", uid);
    retMap.put("timestamp", LocalDateTime.ofInstant(Instant.ofEpochMilli(timeLong),
        Clock.systemDefaultZone().getZone()));
    retMap.put("workerId", workerIdInt);
    retMap.put("dataCenterId", String.valueOf(dataCenterIdInt));
    retMap.put("sequence", sequenceInt);
    return retMap;
  }
   ...
   * format as string
   */
  public String parse2Str(long uid) {
    return JsonUtils.toJSONString(this.parse(uid));
  }
}
