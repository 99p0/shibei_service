package cn.birdplanet.toolkit.core.optauth;
...
public class GoogleAuthenticator {
  ...
  /**
   * set the windows size. This is an integer value representing the number of 30 second windows we
   * allow The bigger the window, the more tolerant of clock skew we are.
   *
  ...
  /**
   * Generate a random secret key. This must be saved by the server and associated with the users
   * account to verify the code displayed by Google Authenticator. The user must register this
   * secret on their device. 生成一个随机秘钥
   *
   * @return secret key
   */
   ...
   * manually enter the secret if desired
   *
   ...
   */
  public static String getQRBarcode(String secret, String account, String issuer) {
